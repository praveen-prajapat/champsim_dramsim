...
vmem_fmtstr = 'VirtualMemory vmem({attrs[size]}, 1 << 12, {attrs[num_levels]}, 22222222, {attrs[minor_fault_penalty]});\n'
...
if original_size <= config_file['num_cores']:
    print("Filling out cores")
    for i in range(original_size, config_file['num_cores']):
        cores.append(copy.deepcopy(cores[0]))
        cores[i]['name'] = 'cpu' + str(i)
        cores[i]['index'] = i
else:
...
with open(instantiation_file_name, 'wt') as wfp:
    ...
    wfp.write('#include "dramsim3_wrapper.hpp"\n')
...
with open(constants_header_name, 'wt') as wfp:
    ...
    print("VIRTUAL WQ OPTIMIZATION ENABLED")
    wfp.write("#define VWQ_ENABLE 1\n")
    
    # ART tracker
    if config_file.get('ART_ENABLE', 0):
        print("[RH_DEFENSE] ART_VR ENABLE")
        wfp.write("#define ART_ENABLE 1\n")
    else:
        wfp.write("#define ART_ENABLE 0\n")
    wfp.write("#define ART_1B " + str(config_file.get('ART_1B', 0)) + "\n")
    wfp.write("#define ART_LITE " + str(config_file.get('ART_LITE', 0)) + "\n")
    wfp.write("#define ART_MM " + str(config_file.get('ART_MM', 0)) + "\n")
    if config_file.get('ART_MM', 0):
        wfp.write("#define ART_CTR_PER_WAY " + str(config_file.get('ART_CTR_PER_WAY', 21)) + "\n")
    else:
        wfp.write("#define ART_CTR_PER_WAY " + str(config_file.get('ART_CTR_PER_WAY', 32)) + "\n")

    if config_file.get('ART_LITE', 0):
        assert(config_file.get('ART_MM', 0))

    # Hydra tracker
    if config_file.get('HYDRA_VR', 0):
        print("[RH_DEFENSE] HYDRA_VR ENABLE")
        wfp.write("#define HYDRA_ENABLE 1\n")
    else:
        wfp.write("#define HYDRA_ENABLE 0\n")
    if config_file.get('GRAPHENE', 0):
        print("[RH_DEFENSE] GRAPHENE ENABLE")
        wfp.write("#define GRAPHENE_ENABLE 1\n")
    else:
        wfp.write("#define GRAPHENE_ENABLE 0\n")
    if config_file.get('PARA', 0):
        print("[RH_DEFENSE] PARA ENABLE")
        wfp.write("#define PARA_ENABLE 1\n")
    else:
        wfp.write("#define PARA_ENABLE 0\n")
    if config_file.get('RAA_RFM', 0):
        print("[RH_DEFENSE] RAA_RFM ENABLE")
        wfp.write("#define RAA_RFM_ENABLE 1\n")
        wfp.write("#define RFM_TH " + str(config_file.get('RFM_TH', 40)) + "\n")
        wfp.write("#define REF_TH " + str(config_file.get('REF_TH', 20)) + "\n")
    else:
        wfp.write("#define RAA_RFM_ENABLE 0\n")
        wfp.write("#define RFM_TH " + str(config_file.get('RFM_TH', -1)) + "\n")
        wfp.write("#define REF_TH " + str(config_file.get('REF_TH', -1)) + "\n")
    if config_file.get('ID_ENABLE', 0):
        print("[RH_DEFENSE] IMPRESS-D ENABLE")
        wfp.write("#define ID_ENABLE 1\n")
    else:
        wfp.write("#define ID_ENABLE 0\n")
    if config_file.get('IP_ENABLE', 0):
        print("[RH_DEFENSE] IMPRESS-P ENABLE")
    ...
    wfp.write("#define HYDRA_RCC_SETS  " + str(config_file.get('HYDRA_RCC_SETS', 4096)) + "\n")
    wfp.write("#define HYDRA_DO_MITS  " + str(config_file.get('HYDRA_DO_MITS', 1)) + "\n")
    # Ideal tracker
    wfp.write("#define IDEAL_TRACKER " + str(config_file.get('IDEAL_TRACKER', 0)) + "\n")

    # BH with ideal tracker
    ...
    wfp.write("#define RH_BLAST_RADIUS " + str(config_file.get('RH_BLAST_RADIUS', 1)) + "\n")

    wfp.write("#define DROP_WB 0\n")
